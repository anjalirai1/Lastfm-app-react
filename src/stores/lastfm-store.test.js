...
describe('LastFm activities actions handlers', () => {
  ...
  describe('lovedTracksRetreived action handler', () => {
    const actionType = handlers.lovedTracksRetreived;
    
    it('should add the lovedTracks to the state', () => {
      const newState = actionType({}, { lovedTracks: 'lovedTracks' });
      expect(newState.lovedTracks)
        .toBe('lovedTracks');
    });

    it('should throw an error if lovedTracks does not exists', () => {
      expect(() => {
        handlers.lovedTracksRetreived({}, {});
      }).toThrowError('No loved tracks');
    });
  });
});
