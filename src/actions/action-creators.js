...
export function fetchLovedTracks(limit) {
  let getLovedTracks = axios.create({
    baseURL,
    url: `?format=json&method=user.getlovedtracks&user=${userName}&limit=${limit}&api_key=${apiKey}`
  });
  getLovedTracks()
  .then((response) => {
    dispatcher.dispatch({
      type: actionTypes.lovedTracksRetreived,
      lovedTracks: response.data.lovedtracks.track
    });
  });
}
